"""
2023년 5월 15일 조병준 작성
goemotion data로 training된 XLMRoberta에 kote를 finetuning 하여 아래 항목을 확인
1. finetuning 시 기존 학습되어 있는 goemotion 성능 감소
2. finetuning 시 kote성능 향상
"""
...
class RoBERTaLitModel(pl.LightningModule):
    ...
    def step(self, x):
        ...
        # metrics 계산(F1 score)
        macro_f1 = torchmetrics.functional.classification.multilabel_f1_score(logits, x['labels'],
                                                                              num_labels=logits.size(-1),
                                                                              average='macro')
        micro_f1 = torchmetrics.functional.classification.multilabel_f1_score(logits, x['labels'],
                                                                              num_labels=logits.size(-1),
                                                                              average='micro')
        weighted_f1 = torchmetrics.functional.classification.multilabel_f1_score(logits, x['labels'],
                                                                                 num_labels=logits.size(-1),
                                                                                 average='weighted')
    ...
    def configure_optimizers(self):
        ...
        train_step = (len(self.train_dataloader()) // n_gpus) + 1 if len(self.train_dataloader()) % n_gpus else len(
            self.train_dataloader()) // n_gpus
    ...
    def validation_epoch_end(self, outputs):
        all_gather = self.all_gather(outputs)  # outputs = [local rank 0, local rank 1]
        ...
        if self.local_rank == 0:  # only 1 process running
            for i in range(2):
                ...
                for key in report.keys():
                    mydict = {['goemotion', 'kote'][i]+"_val_" + key + "_" + k: torch.tensor(v, dtype=torch.float32) for k, v in
                              report[key].items()}
                    self.log_dict(mydict, prog_bar=False, rank_zero_only=True)

    def prepare_goemotion(self):
        ...
        # tokenize
        train_encodings = self.tokenizer(goemotions['train']['text'],
                                         padding="max_length",
                                         truncation=True,
                                         return_tensors="pt",
                                         )
        val_encodings = self.tokenizer(goemotions['validation']['text'],
                                       padding="max_length",
                                       truncation=True,
                                       return_tensors="pt",
                                       )
        test_encodings = self.tokenizer(goemotions['test']['text'],
                                        padding="max_length",
                                        truncation=True,
                                        return_tensors="pt",
                                        )
        ...
        for m in ['train', 'validation', 'test']:
            tmp = []
            labels = goemotions[m]['labels']

            for label in labels:
                ekman_ = set()
                for l in label:  # multilabel
                    for k, v in ekman_mapping.items():
                        if go_idx2label[l] in v:
                            ekman_.add(ekman_label2idx[k])
        ...
        train_labels = mlb.fit_transform(label_dict["train"])
        test_labels = mlb.fit_transform(label_dict["test"])
        val_labels = mlb.fit_transform(label_dict["validation"])

        return [train_encodings, train_labels], [val_encodings, val_labels], [test_encodings, test_labels]

    def prepare_kote(self):
        # preprocessing KOTE datasets
        ...
        # tokenize
        train_encodings = self.tokenizer(kote_ekman['train']['text'],
                                         padding="max_length",
                                         truncation=True,
                                         return_tensors="pt",
                                         )
        val_encodings = self.tokenizer(kote_ekman['validation']['text'],
                                       padding="max_length",
                                       truncation=True,
                                       return_tensors="pt",
                                       )
        test_encodings = self.tokenizer(kote_ekman['test']['text'],
                                        padding="max_length",
                                        truncation=True,
                                        return_tensors="pt",
                                        )
    ...
    def prepare_data(self) -> None:
        # load goemotions and kote datasets
        self.goemotions_datasets = self.prepare_goemotion()
        self.kote_datasets = self.prepare_kote()

    def setup(self, stage):
        if stage == 'fit':
            # goemotions
            self.go_train_ds = GoEkmanDataset(*self.goemotions_datasets[0])
            self.go_val_ds = GoEkmanDataset(*self.goemotions_datasets[1])

            # kote
            self.kote_train_ds = KOTEEkmanDataset(*self.kote_datasets[0])
            self.kote_val_ds = KOTEEkmanDataset(*self.kote_datasets[1])

            # concatenate datasets
            self.concat_ds = torch.utils.data.ConcatDataset([self.go_train_ds, self.kote_train_ds])

        if stage == 'test':
            self.go_test_ds = GoEkmanDataset(*self.goemotions_datasets[2])
            self.kote_test_ds = KOTEEkmanDataset(*self.kote_datasets[2])

    def train_dataloader(self):
        # return DataLoader(self.concat_ds, shuffle=True, pin_memory=True, num_workers=2,
        #                   batch_size=self.opt.batch_size)
        return DataLoader(self.kote_train_ds, shuffle=True, pin_memory=True, num_workers=2,
                          batch_size=self.opt.batch_size)

    def val_dataloader(self):
        goemotion_dl = DataLoader(self.go_val_ds, shuffle=False, pin_memory=True, num_workers=2,
                           batch_size=self.opt.batch_size)
        kote_dl = DataLoader(self.kote_val_ds, shuffle=False, pin_memory=True,
                          num_workers=2, batch_size=self.opt.batch_size)

        return goemotion_dl, kote_dl

    def test_dataloader(self):
        goemotion_dl = DataLoader(self.go_test_ds, shuffle=False, pin_memory=True, num_workers=2,
                                  batch_size=self.opt.batch_size)
        kote_dl = DataLoader(self.kote_test_ds, shuffle=False, pin_memory=True,
                             num_workers=2, batch_size=self.opt.batch_size)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default='go->kote_v1', help='wandb logger model name')
